...
internal object FollowLocationPolicy {
    ...
    const val CAMERA_FIX_MAX_AGE_MILLIS = 10_000L
    const val CAMERA_MAXIMUM_ACCURACY_METERS = 75.0
    const val SEARCH_ORIGIN_MAX_AGE_MILLIS = 120_000L
    const val MAXIMUM_FUTURE_SKEW_MILLIS = 5_000L
    const val SEARCH_BIAS_MAXIMUM_ACCURACY_METERS = 10_000.0

    fun shouldRepoll(location: NavigationLocation?, nowEpochMillis: Long): Boolean =
        location == null || !isUsable(
            location,
            nowEpochMillis,
            REPOLL_AFTER_MILLIS,
            CAMERA_MAXIMUM_ACCURACY_METERS,
        )
    ...
    fun canBiasSearch(location: NavigationLocation, nowEpochMillis: Long): Boolean =
        isUsable(
            location,
            nowEpochMillis,
            SEARCH_ORIGIN_MAX_AGE_MILLIS,
            SEARCH_BIAS_MAXIMUM_ACCURACY_METERS,
        )
    ...
    private fun isUsable(
        ...
        location.accuracyMeters in 0.0..maximumAccuracyMeters &&
