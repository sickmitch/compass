...
def test_phase5_batches_only_postgis_pruned_candidates(
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            assert result.spatial_result.metrics.active_station_count == 4
            assert result.spatial_result.metrics.active_station_with_location_count == 3
            assert result.spatial_result.metrics.corridor_candidate_count == 2
            assert result.metrics.matrix_candidate_count == 2
            ...
            assert {
                candidate.station.mimit_station_id for candidate in result.candidates
            } == {"1001", "1002"}
            assert all(
                candidate.distance_from_previous_waypoint_meters == 100_000
