...
result_path="/tmp/compass-traffic-plan-live.json"
status_path="/tmp/compass-traffic-plan-valhalla-status.json"
validator="$repo_root/scripts/validate-traffic-plan-live.py"
...
echo "[2/8] Building the pinned Compass traffic updater"
...
echo "[4/8] Preparing TomTom base Flow Segment probes in forced dry-run mode"
...
echo "[5/8] Recording traffic.tar checksum before planning"
traffic_hash_before="$(
    docker compose --profile routing exec -T valhalla \
    sha256sum /custom_files/traffic.tar | awk '{print $1}'
)"

echo "[6/8] Planning whole-edge set/reset operations without applying them"
docker compose --profile traffic run --rm --no-deps traffic-updater \
compass-traffic plan-once --limit 10 >"$result_path"

echo "[7/8] Validating speeds, closures, expiry, GraphIds and identities"
python3 "$validator" --response "$result_path"

echo "[8/8] Proving that planning did not modify traffic.tar"
traffic_hash_after="$(
    docker compose --profile routing exec -T valhalla \
    sha256sum /custom_files/traffic.tar | awk '{print $1}'
)"
...
echo
echo "READ-ONLY TRAFFIC EDGE PLAN CHECK COMPLETED"
echo "Full result: $result_path"
echo "No provider speed was written and no updater state was persisted."
