...
if [[ -n "${DESTINATION_SEARCH_DEBOUNCE_MS:-}" ]]; then
    gradle_properties+=(-PDESTINATION_SEARCH_DEBOUNCE_MS="$DESTINATION_SEARCH_DEBOUNCE_MS")
fi
if [[ -n "${DESTINATION_SEARCH_MIN_CHARS:-}" ]]; then
    gradle_properties+=(-PDESTINATION_SEARCH_MIN_CHARS="$DESTINATION_SEARCH_MIN_CHARS")
fi
...
echo "[2/4] Building Android 0.19.4"
