...
installable="$repo_root/dist/Compass-0.28.2-debug-installabile.apk"
...
echo "[2/5] Running Android tests, lint and build"
(
    cd "$android_root"
...
echo "[3/5] Installing Android 0.28.2 without clearing private app data"
"$adb" install -r "$apk"
...
OPERATOR CHECK — ORDINARY-STOP ADDED-TIME LIMIT

Run all five cases in:
  docs/phases/android-0.28.2-intermediate-stop-time-limit-acceptance.md

The critical invariant is that the exact Valhalla added driving duration, not added distance,
controls ordinary-stop eligibility. The default is one third of the direct driving duration and a
custom minute value overrides it. CNG detour handling must remain unchanged.
...
"$adb" logcat -d -t 1000 | grep -E 'Compass(Api|Navigation)|AndroidRuntime' | tail -n 180 || true

echo "ANDROID 0.28.2 INTERMEDIATE-STOP TIME-LIMIT LIVE GATE COMPLETE"
echo "Installable APK: $installable"
sha256sum "$installable"
echo "Return this complete output and pass/fail notes for all five checks."
