...
installable="$repo_root/dist/Compass-0.28.1-debug-installabile.apk"
...
echo "[3/5] Installing Android 0.28.1 without clearing private app data"
"$adb" install -r "$apk"
...
OPERATOR CHECK — NAVIGATION CONTROLS AND MUTE PERSISTENCE

Run all seven cases in:
  docs/phases/android-0.28.1-navigation-controls-acceptance.md

The critical invariant is that a muted active navigation remains muted after route replacement and
process recreation. Red must remain reserved for actual error states.
...
echo "ANDROID 0.28.1 NAVIGATION CONTROLS LIVE GATE COMPLETE"
