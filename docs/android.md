...
favourites and encrypted server-connection stores, and loads
...
maneuver card uses a 70% surface opacity, the left voice/options controls align with the map
...
The structured green exit sign is rendered outside and below the maneuver card, aligned to its
right edge. The upcoming CNG-stop card shares the same 70% glass opacity. The navigation voice
button now keeps a stable dark-surface/mint-content palette in both states and communicates state
only through the volume glyph and accessibility description. The trip summary no longer duplicates
the aggregate CNG-stop count immediately above the detailed CNG-stop list.
...
Android `0.28.28` (`versionCode=74`) adds the persistent **Autostrade** option. Disabled requests
exclude motorways from the first calculation onward. Enabled requests permit them but show a
mandatory confirmation when Valhalla reports actual motorway use; choosing **Ricalcola senza**
applies only to the current route and its later reroutes, without changing the saved option.
